...
import numpy as np
# simulate network
from plasticattractor_networksims_TMSSHFL import plasticattractor_sim_SHFL
# behavioural analysis 
from plasticattractor_behaviouralanalysis import plasticattractor_beh
# behavioural analysis TMS 
from plasticattractor_behaviouralanalysis_TMSSHFL import plasticattractor_beh_TMSSHFL
# decoding analysis 
...
TMS_sim = np.zeros([num_blocks,num_trials+2])

for rnd in range(n_seeds):
    conj[rnd], feat[rnd], choice[rnd], acc[rnd],rt[rnd],\
    c_lab[rnd], s_lab[rnd], r_lab[rnd], weights[rnd]\
    = plasticattractor_sim_SHFL(num_blocks, num_trials, rnd, TMS_sim, TMS_start = None)
...
# trials x blocks matrix
TMS = np.tile([0,1],int(num_trials/2))
TMS_sim = np.zeros([num_trials+2,num_blocks])
for col_idx in range(num_blocks):
    np.random.shuffle(TMS)
    TMS_sim[2:,col_idx] = TMS
...
for rnd_seed in range(n_seeds):
    for i, tms_start in enumerate(tms_vals):
        ...
        = plasticattractor_sim_SHFL(num_blocks, num_trials, rnd_seed, TMS_sim, TMS_start = (100-tms_start))
        prog_counter += 1
...
for tms in range(len(tms_vals)):
    for rnd in range(n_seeds):
        ...
        = plasticattractor_beh_TMSSHFL(num_trials, num_blocks, acc_tms[rnd,tms], rt_tms[rnd,tms], c_lab_tms[rnd,tms], s_lab_tms[rnd,tms], r_lab_tms[rnd,tms], conj_tms[rnd,tms], feat_tms[rnd,tms],TMS_sim)
    ...
    mean_cong_diff_rt = np.mean(np.array(list(incongruent_rt_tms.values())) - np.array(list(congruent_rt_tms.values())))
    ...
    ster_cong_diff_rt = np.std(np.array(list(incongruent_rt_tms.values())) - np.array(list(congruent_rt_tms.values())))/np.sqrt(n_seeds)
        
# %% Weight matrix eigenvalue analysis

# basic simulations
sat_vals = np.zeros([n_seeds,num_trials, num_blocks])
largest_val =  np.zeros([n_seeds, num_trials, num_blocks])
for rnd in range(n_seeds):
    for blk in range(num_blocks):
        for trl in range(num_trials):
            ...
            largest_val[rnd,trl,blk] = np.max(vals)
        
mean_sat_vals = np.mean(np.mean(sat_vals,0),1)
mean_largest_val = np.mean(np.mean(largest_val,0),1)

# tms simulations
sat_vals_tms = np.zeros([n_seeds, num_trials, num_blocks, len(tms_vals)])
largest_val_tms =  np.zeros([n_seeds, num_trials, num_blocks, len(tms_vals)])
W_spread =  np.zeros([n_seeds, num_trials, num_blocks, len(tms_vals)])
W_entropy =  np.zeros([n_seeds, num_trials, num_blocks, len(tms_vals)])
for tms in range(len(tms_vals)):
    for rnd in range(n_seeds):
        for blk in range(num_blocks):
            for trl in range(num_trials):
                W = weights_tms[rnd,tms][trl,blk]
                # W = W[2:,:,-1]
                W_spread[rnd,trl,blk,tms] = np.mean(np.std(W[0:3,:],0))
                W_entropy[rnd,trl,blk,tms] = sum(sum(-(W/np.sum(W,0))*np.log2(W/np.sum(W,0))))
                [vals, vecs] = np.linalg.eig(W@W.T)
                # number of saturating eigen values (i.e. >1)
                sat_vals_tms[rnd,trl,blk,tms] = np.sum(vals>1)
                largest_val_tms[rnd,trl,blk,tms] = np.max(vals)
            
mean_W_spread = np.mean(np.mean(W_spread,0),1)
mean_W_entropy = np.mean(np.mean(W_entropy,0),1)
mean_sat_vals_tms = np.mean(np.mean(sat_vals_tms,0),1)
mean_largest_val_tms = np.mean(np.mean(largest_val_tms,0),1)  
ster_sat_vals_tms = np.std(np.mean(sat_vals_tms,0),1)/np.sqrt(n_seeds)
ster_largest_val_tms = np.std(np.mean(largest_val_tms,0),1)/np.sqrt(n_seeds)

# # time resolved analysis
# sat_vals_tms_t = np.zeros([n_seeds, num_trials, num_blocks, len(tms_vals),400])
# for tms in range(len(tms_vals)):
#     for rnd in range(n_seeds):
#         for blk in range(num_blocks):
#             for trl in range(num_trials):
...
#                 for t in range(400):
#                     W_t = W[:,:,t]
#                     [vals, vecs] = np.linalg.eig(W_t@W_t.T)
#                     # number of saturating eigen values (i.e. >1)
#                     # sat_vals_tms_t[rnd,trl,blk,tms,t] = np.sum(vals>1)
#                     sat_vals_tms_t[rnd,trl,blk,tms,t] = np.sum(vals>1)
...
plt.errorbar(tms_strength[1:],100*mean_accuracy_tms[1:], yerr = 100*ster_accuracy_tms[1:], color = 'black', capsize=3, linestyle="-",marker="o")
plt.xlabel("Train length")
plt.ylabel("Accuracy %")
plt.xticks(np.arange(0,tms_max,1))
...
plt.errorbar(tms_strength[1:],mean_rt_tms[1:], yerr = ster_rt_tms[1:],color = 'black', capsize=3, linestyle="-",marker="o")
plt.xlabel("Train length")
...
plt.errorbar(tms_strength,100*mean_congruent_acc_tms, yerr = 100*ster_congruent_acc_tms, color = 'indianred', capsize=3, linestyle="-",marker="o")
plt.errorbar(tms_strength,100*mean_incongruent_acc_tms, yerr = 100*ster_incongruent_acc_tms, color = 'darkred', capsize=3, linestyle="-",marker="o")
plt.xticks(np.arange(0,tms_max,1))
...
fig = plt.figure()
# plt.title('Train length vs congrunecy effect')
plt.errorbar(tms_strength[1:],mean_congruent_rt_tms[1:], yerr = ster_congruent_rt_tms[1:], color = 'indianred', capsize=3, linestyle="-",marker="o")
plt.errorbar(tms_strength[1:],mean_incongruent_rt_tms[1:], yerr = ster_incongruent_rt_tms[1:], color = 'darkred', capsize=3, linestyle="-",marker="o")
plt.xticks(np.arange(0,tms_max,1))
fig.legend(['Congruent','Incongruent'])
plt.xlabel("Train length")
...
data = mean_incongruent_rt_tms[1:] - mean_congruent_rt_tms[1:]
error = ster_incongruent_rt_tms[1:] - ster_congruent_rt_tms[1:]
fig = plt.figure()
plt.title('Congruency effect vs TMS train length')
plt.bar(labels,data, color = 'lightgrey', capsize=3)
plt.xlabel("Train length")
plt.ylabel("Reaction times (time-steps)")
...
# congruent vs incongruent reaction time box plot TMS = 11
tms = 10
...
plt.errorbar(tms_strength[0:],mean_sat_vals_tms[0,0:], yerr= ster_sat_vals_tms[0,0:], color = 'black', capsize=3, linestyle="-",marker="o")
plt.errorbar(tms_strength[0:],mean_sat_vals_tms[5,0:], yerr= ster_sat_vals_tms[5,0:], color = 'blue', capsize=3, linestyle="-.",marker="o")
plt.errorbar(tms_strength[0:],mean_sat_vals_tms[11,0:], yerr= ster_sat_vals_tms[11,0:], color = 'red', capsize=3, linestyle="-.",marker="o")
plt.xlabel("Strength")
plt.ylabel("Eigenvalues > 1")
plt.legend(['Trial 1', 'Trial 6', 'Trial 12' ])
plt.xticks(np.arange(0,tms_max,1))

# weight spread vs TMS line plot
fig = plt.figure()
# plt.title('TMS train length vs number of saturating eigenvalues')
plt.plot(tms_strength[0:],mean_W_spread[0,0:], color = 'black', linestyle="-",marker="o")
plt.plot(tms_strength[0:],mean_W_spread[5,0:], color = 'blue', linestyle="-.",marker="o")
plt.plot(tms_strength[0:],mean_W_spread[11,0:], color = 'red', linestyle="-.",marker="o")
plt.xlabel("Strength")
plt.ylabel("W std")
plt.legend(['Trial 1', 'Trial 6', 'Trial 12' ])
plt.xticks(np.arange(0,tms_max,1))
...
# fig = plt.figure()
# trl = 1;
# plt.plot(sat_vals_tms_t[0,4,trl,1,:],  color = 'black')
# plt.plot(sat_vals_tms_t[0,4,trl,10,:], color = 'blue')
# plt.plot(sat_vals_tms_t[0,4,trl,19,:], color = 'red')
# plt.xlabel("time")
# plt.ylabel("Eigenvalues > 1")
...
# relevant
fig = plt.figure()
plt.scatter(np.mean(data[0:],1),np.mean(rel_meandiff[0:],1), color = 'lightgrey')
plt.xlabel("Average congruency effect")
plt.ylabel("Average difference in relevent firing rate (abs)")

# irrelevant
fig = plt.figure()
plt.scatter(np.mean(data[0:],1),np.mean(irrel_meandiff[0:],1), color = 'lightgrey')
plt.xlabel("Average congruency effect")
...
norm = mpl.colors.Normalize(vmin=0, vmax=tms_max)
cmap2 = mpl.cm.ScalarMappable(norm=norm, cmap=mpl.cm.get_cmap('Greys'))
cmap2.set_array([])
plt.colorbar(cmap2, ticks=np.arange(0,tms_max)).ax.invert_yaxis() 
plt.xlabel("Time")
...
congruent_irreldiff_correlation = np.corrcoef(np.mean(data[0:],1),np.mean(irrel_meandiff[0:],1))
