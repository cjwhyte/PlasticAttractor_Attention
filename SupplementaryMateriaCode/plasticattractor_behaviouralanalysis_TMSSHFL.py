...
def plasticattractor_beh_TMSSHFL(num_trials, num_blocks, acc_dict, rt_dict, c_label_dict, s_label_dict, r_label_dict,conj_dict, feat_dict,TMS_sim):
    ...
    rt = np.zeros([int(num_trials), num_blocks])
    accuracy = np.zeros([int(num_trials), num_blocks])
    for blk in range(num_blocks):
        TMS_idx = TMS_sim[blk,:]==1
        # accuracy
        blk_acc = acc_dict[blk]
        accuracy[:,blk] = blk_acc[2:]
        # rt
        blk_rt = rt_dict[blk]
        rt[:,blk] = blk_rt[2:]
        
    TMS_trl_idx = TMS_sim[:,2:]    
    TMS_idx = TMS_trl_idx.flatten()==1
    
    # overall accuracy
    accuracy_overall = accuracy.flatten()
    accuracy_overall =  accuracy_overall[TMS_idx]
    accuracy_overall = sum(accuracy.flatten())/(num_trials/2*num_blocks)

    # rt (for correct trials only)
    accuracy_flat = accuracy.flatten()
    rt = rt.flatten()
    correct_idx = (accuracy_flat == 1)
    correct_idx = .5*correct_idx + .5*TMS_idx
    ...
    sorted_c_labels = np.zeros([int(num_trials),num_blocks])
    sorted_s_labels = np.zeros([int(num_trials),num_blocks])
    ...
    congruent_accuracy = sum(accuracy_flat[congruent_idx])/(.5*(int(num_trials/4)*num_blocks))
    incongruent_accuracy = sum(accuracy_flat[incongruent_idx])/(.5*(int(num_trials/4)*num_blocks))
